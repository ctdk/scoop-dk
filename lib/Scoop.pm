...
sub initialize {
	...
	# and the new story caching object
	$self->_set_story_cache();
...
sub story_cache         { return $_[0]->{STORY_DATA_CACHE}   }
...
sub page_out {
	...
	# return if we've already spit a page out. Shouln't happen too often,
	# though.
	if ($S->{NO_PAGE_OUT}){
		$S->{NO_PAGE_OUT} = 0; # don't want it hanging around
		return;
		}
	...
	if ($S->{UI}->{VARS}->{no_cache} && !$S->{ALLOW_CACHING}){
	...
	# do the ol' switcheroo. Make this more general eventually
	my $swi = 0;
	my $sw = sub  { unless($swi){$swi++;}else{$swi--;} return ($swi) ? "images.dailykos.com" : "images2.dailykos.com"; };
        $S->{FORM} =~ s/images.dailykos.com/$sw->()/meg;
	...
	unless ($S->{FILTERED}) {
		...
		while (my $p = substr($S->{'FORM'}, $start, $len)) {
			$start += $len;
			#$time = localtime(time());
			warn "  (Scoop::page_out $time) Start Printing response page\n" if $DEBUG;
			$S->{APACHE}->print($p);
			#$S->{APACHE}->rflush();
			#$time = localtime(time());
			warn "  (Scoop::page_out $time) Done Printing response page\n" if $DEBUG;
		}
		if ($S->{UID} == -1 && !$S->{SESSION_KEY} && ($S->cgi->param('op') eq 'displaystory' || $S->cgi->param('op') eq 'main' || $S->cgi->param('op') eq 'comments') && $S->{UI}->{VARS}->{expercache}){
                        # An experiment
                        my $uri = $S->apache->uri();
                        $S->cache->store("mcuri_$uri", $S->{FORM}, "+2m");
                        }
		$S->{APACHE}->rflush();
	} else {
		print $S->{FORM};
...
sub _load_ajax_tools {
        my $self = shift;
        my $time = time();

        if (my $cached = $self->cache->fetch_data({resource => 'ajax_tools',
                                                   element  => 'AJAX_TOOLS'}))
{
                $self->{AJAX_TOOLS} = $cached;
                return $self;
        }

        warn "Reloading ajax tools cache.\n" if $CACHE_DEBUG;
        ...
        while (my $tool = $sth->fetchrow_hashref()) {
                $self->{AJAX_TOOLS}->{ $tool->{tool} } = $tool;
        }
        $sth->finish;
        $self->cache->cache_data({resource => 'ajax_tools',
                                  element  => 'AJAX_TOOLS',
                                                          data     => $self->{AJAX_TOOLS}});
...
sub _set_dbh {
	...
	# this way we don't actually open a separate connection, but can
	# test on whether the archive's there or not
	...
	if ($dbhost) {
		...
		# Make it part of the object
		$self->{DBHARCHIVE} = 1; # $dbh; # there's the occasional test
					 # on DBHARCHIVE
		$self->{HAVE_ARCHIVE} = 1;
	} else {
		$self->{HAVE_ARCHIVE} = 0;
	...
	#if($self->memcached->get("dbdump_lck")){
	#	$dbhost = '';
	#	}
        if($dbhost){
...
sub _check_user {
	...
	$self->{TRUSTLEV} = $user->{'trustlev'} || 1;
	if($self->{GID} eq 'Banned' || $self->{GID} eq 'Anonymous'){
                $self->{TRUSTLEV} = 0;
                }
...
sub _set_prefs {
	...
	if ($force) {
		...
		#$self->cache->remove("ud_" . $self->{UID});
	}
...
sub _set_memcached {
        ...
        my $memd = new Cache::Memcached::Fast({ servers => \@memarr , nowait => 1, compress_threshold => $memcomp } );
...
sub _set_story_cache {
        my $self = shift;
        my $story_cache = Scoop::StoryCache->new($self);
        $self->{STORY_DATA_CACHE} = $story_cache;
        return;
        }
...
sub check_for_login {
	...
	# WTF. Why weren't we bailing if none of $uname, $pass, and $mail were
        # set??
        return $uid if (!$uname && !$pass && !$mail);
...
sub check_password {
	...
	my $fm = ($S->cgi->param('firstlogin') || $S->{FIRSTLOG}) ? 1 : 0;
...
sub user_data {
	...
	if (ref($uid) !~ /ARRAY/) {
		...
		if ($S->{USER_DATA_CACHE}->{$uid}) { # ||= $S->cache->fetch("ud_$uid")) {
	...
	foreach my $u (@{$uid}) {
		...
		#$S->cache->remove("ud_$uid");
		unless ($S->{USER_DATA_CACHE}->{$u}){ # ||= $S->cache->fetch("ud_$uid")){
			next if (($S->{UID} != $u) && ($S->{USER_DATA_CACHE}->{$u} = $S->cache->fetch("udata_$u")));
	...
	# make a temp hash to stick all this into for putting into memcached
	my $tmpud = {};
	($rv, $sth) = $S->db_select($get);
	while (my $u = $sth->fetchrow_hashref()) {
		# Handle anonymous specially...
		warn "In user_data: Caching $u->{uid}\n" if $DEBUG;
		$tmpud->{$u->{uid}} = $S->{USER_DATA_CACHE}->{ $u->{uid} } = $u;
	}
	$sth->finish();
	...
	while (my $u = $sth->fetchrow_hashref()) {
		...
		$tmpud->{$u->{uid}}->{prefs}->{$u->{prefname}} = $S->{USER_DATA_CACHE}->{$u->{uid}}->{prefs}->{$u->{prefname}} = $u->{prefvalue};
	}
	$sth->finish();
	...
	if ($subs) {
		($rv, $sth) = $S->db_select($subs);
		while (my $u = $sth->fetchrow_hashref()) {
			$tmpud->{$u->{uid}}->{sub} = $S->{USER_DATA_CACHE}->{$u->{uid}}->{sub} = $u;
		}
		$sth->finish();
	}	
	# put the stuff from $tmpud into memcached
	#foreach my $x (keys %{$tmpud}){
	#	$S->cache->store("ud_$x", $tmpud->{$x});
	#	}
	foreach my $ud (@filtered_users){
                next if $ud == $S->{UID};
                $S->cache->store("udata_$ud", $S->{USER_DATA_CACHE}->{$ud}, "+15m");
                }
...
sub mail {
	...
	my $filt = $S->filter_param($Mail::Sendmail::error);
	$error =~ s/%%error%%/$filt/;
	return $error;
}
...
sub date_format {
	...
	} elsif ($format eq 'RSS2'){
                $date_format = "%a, %d %b %Y %H:%i:%S GMT";
