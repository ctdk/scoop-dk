...
sub populate_last_seen {
	...
	my $ls = $S->cache->fetch("ls_" . $S->{UID} . "$sid");
	if($ls){
		$S->{LAST_SEEN}->{$ls->{sid}} = $ls;
		return;
		}
	#$S->{LAST_SEEN} = {};
	...
	while (my $item = $sth->fetchrow_hashref()) {
		...
		$S->cache->store("ls_" . $S->{UID} . "$sid", $item);
	}
	$sth->finish;
}
...
sub update_seen {
	...
	my ($rv, $sth) = $S->db_insert({
		...
		COLS   => 'uid, sid, lastseen, highest_idx',
		VALUES => qq|$S->{UID}, $q_sid, $count, $highest|,
		DUPLICATE => qq|lastseen = $count, highest_idx = $highest|,
		DEBUG => 0
		});
	$sth->finish;
	...
	# clear the memcached version
	$S->cache->remove("ls_" . $S->{UID} . "$sid");
