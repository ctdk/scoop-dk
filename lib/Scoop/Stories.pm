...
sub main_page {
	...
	my $op = shift; # the op
        ...
        if ($op eq 'section' || $op eq 'crawl') {
                $op = 'section'; # should do
                }
	if ($op eq 'section') {
...
sub special {
	...
	my $page = $S->cache->fetch("special_" . $id);
	if(!$page) {
		$page = $S->_get_special_page($id);
		$S->cache->store("special_" . $id, $page);
		}
...
sub submit_story {
	...
	# make sure we're not submitting more than the hard limit of stories or
	# diaries.
	if($S->{UI}->{VARS}->{'use_hard_story_limit'} && !$S->have_perm('evade_story_hard_limit')){
		# find out if we've posted in the past day
		my ($rv, $sth) = $S->db_select({
			WHAT => 'count(*)',
			FROM => 'stories',
			WHERE => "(TO_DAYS(NOW()) - TO_DAYS(time) < 1) AND aid = $S->{UID} AND displaystatus >= -1"
			});
		my $storynum = $sth->fetchrow();
		$sth->finish;
		unless ($storynum < $S->{UI}->{VARS}->{'hard_story_limit'}){
			# If they've already met the posting limit, tell them
			# that they can't post.
			my $dplural = ( $S->{UI}->{VARS}->{'hard_story_limit'} == 1) ? 'y' : 'ies';
			$S->{UI}->{BLOCKS}->{subtitle} = "Sorry!";
			$S->{UI}->{BLOCKS}->{CONTENT} = "Sorry, but you can only post $S->{UI}->{VARS}->{'hard_story_limit'} diar$dplural on this site per day. Come back again tomorrow.";
			return;
			}
		}
...
sub _check_story_mode {
	...
	#$S->{STORIES}->{$sid}->{displaystatus} = $sd->[11] unless $S->{STORIES}->{$sid}->{displaystatus};
	unless(defined($S->{STORIES}->{$sid}->{displaystatus})){
                my $sd = $S->story_basic_data_arr($sid);
                $S->{STORIES}->{$sid}->{displaystatus} = $sd->[11];
                }
...
sub _check_archivestatus {
	...
	my $sid = shift;
	my $stat;
	$stat = $S->cache->fetch("astat_$sid");
	if($stat){
		$S->{STORIES}->{$sid}->{archivestatus} = ($stat eq '0E0') ? 0 : $stat;
		}
	...
	if ($S->{DBHARCHIVE}) {
	my $q_sid = $S->dbh->quote($sid);
	my $where = ($sid =~ /\D/) ? "sid = $q_sid" : "id = $q_sid";
	...
	my $store = ($stat == 0) ? '0E0' : $stat;
	$S->cache->store("astat_$sid", $store, "+15m");
	return $stat;
}
...
sub _get_story_section {
	...
	my $sid = shift;
	my $quoted_sid = $S->dbh->quote($sid);
	#if (my $sd = $S->story_basic_data_arr($sid)){
	#	return $sd->[10];
	#	}
