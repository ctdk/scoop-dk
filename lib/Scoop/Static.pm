...
sub check_do_static {
	...
	return 0 unless ($S->cgi()->param('op') eq 'displaystory'); #&& $S->cgi->param('caller_op') ne 'storyonly');
...
sub make_static_path {
	...
	my $base_path = ($S->cgi->param('caller_op') ne 'storyonly') ? $S->{UI}->{VARS}->{page_path} : $S->{UI}->{VARS}->{page_so_path};
	my $sid = $S->cgi->param('sid');
...
sub make_cache_dir_path {
	...
	my $p = ($S->cgi->param('caller_op') ne 'storyonly') ? $S->{UI}->{VARS}->{page_path} : $S->{UI}->{VARS}->{page_so_path};
	$path =~ s/^$p\///;
	my $pre_path = ($S->cgi->param('caller_op') ne 'storyonly') ? $S->{UI}->{VARS}->{page_path} : $S->{UI}->{VARS}->{page_so_path};
	my @elem = split '/', $path;
	pop @elem;
