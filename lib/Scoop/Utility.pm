...
sub get_file_upload {
	...
	if($Scoop::MP2){
		warn "In upload...\n";
		warn "file upload: " . $S->cgi->param('file_upload') . "\n";
...
sub auto_format {
	...
	# dkosopedia formatting stuff.
	# Put in dk{{Whatever phrase|link text}}, and it turns it into a link to
	# http://www.dkosopedia.com/index.php/Whatever_phrase
	# Pretty simple, really
	while(/dk{{(.*?)}}/g){
		my ($term, $linktext) = split /\|/, $1;
		$linktext ||= $term;
		$term =~ s/ /_/g;
		my $htmllink = qq|<a class="dk" href="http://www.dkosopedia.com/index.php/$term">$linktext</a>|;
		s/dk{{(.*?)}}/$htmllink/;
		}
...
sub url_shorten {
        my $S = shift;
	my $comment = shift;
	return $comment if !$S->{UI}->{VARS}->{shorten_url};
	$comment =~ s/(<a.+?href[^>]+>)\s*(.+?)\s*(<\/a>)/$1 . $S->_shorten_url_text($2) . $3/gei;
        return $comment;
        }

sub _shorten_url_text {
        my $S = shift;
        my $input = shift;
        return $input if $input !~ m#(^http(s)?://|^ftp://)#;
        my @tok = split '/', $input;
...
sub filter_param {
        my $S = shift;
        my $val = shift;
        my $ret = $S->filter_subject($val);
	$ret =~ s/"/&#34;/g;
	$ret =~ s/'/&#39;/g;
	return $ret;
        }

sub get_story_id_from_sid {
	my $S = shift;
	my $sid = shift;
	my $id = $S->{ID_CACHE}->{$sid} || $S->cache->fetch("s_id_f_sid_$sid");
	return $id if $id;
	my $qsid = $S->dbh->quote($sid);
	# FIXME: make a cache later, yo
	...
	$S->{ID_CACHE}->{$sid} = $id;
	$S->cache->store("s_id_f_sid_$sid", $id) if ($id);
	return $id;
	}

sub get_sid_from_story_id {
        my $S = shift;
        my $id = shift;
        my $sid = $S->{ID_CACHE}->{$id} || $S->cache->fetch("s_sid_f_id_$id");
        return $sid if $sid;
	...
	$S->{ID_CACHE}->{$id} = $sid if ($sid);
        $S->cache->store("s_sid_f_id_$id", $sid) if ($sid);
        return $sid;
        }

sub moduloze {
	# for making the filler in a URL between the sid and story id
	# consistently
	my $S = shift;
	my $id = shift;
	# why 1017 and 1011? I dunno, just seemed like OK numbers
	$id = ($id % 1017) ^ 1011;
	return $id;
	}

# craft up URL's for the fancy new URL scheme

sub urlcraft {
        my $S = shift;
        my $id = shift;
	my $url = $S->cache->fetch("kurl_$id");
	#return $url if $url;
        my $sd = $S->story_basic_data_arr($id);
        $sd->[0] =~ m#^(.*?)/(.*?)/(.*?)/#;
        my $date = "$1/$2/$3";
        my $title = $sd->[3];
        $title = $S->filter_param($title);
	$title =~ s/&(.*?);//g;
        $title =~ s#/#-#g;
        $title =~ s/ /-/g;
        $title =~ s/\?//g;
	...
	$title =~ s/%//g;
	$title =~ s/-{2,}//g;
        $url = "$date/$sd->[26]/-$title";
	$S->cache->store("kurl_$id", $url);
