...
sub _delete_poll {
	...
	$S->cache->remove("poll_hash_$unquote_qid");
	$S->cache->remove("poll_answers_$unquote_qid");
...
#	my $qid = $S->_generate_unique_qid();
	...
	my ($rv, $sth) = $S->db_insert({
		DEBUG   => 0,
		INTO    => 'pollquestions',
		VALUES  => qq|$newqid, $question, $voters, $current_time, 0, $is_multiple_choice, $pub |,
	});
	$sth->finish;
...
sub _update_poll {
	my $S = shift;
	my $sid = shift; #it'll come in handy at least
	...
	$newqid = $S->dbh->quote($newqid);
	$current_time = $S->dbh->quote($current_time);
	# hmph. I don't believe this, but we need the goddamned story_data
	# and poll hashes, or else this won't work
	my $pub;
	my $sd = $S->story_data_arr([$sid]);
	my $ph = $S->get_poll_hash($sd->[0]->[16]);
	if($S->cgi->param('operat')){
		if ($S->cgi->param('operat') eq 'draft'){
			$pub = $ph->{unpublished};
			}
		else {
			$pub = ($S->cgi->param('question')) ? 0 : 1;
			}
		}
	else {
		# doing it the old way. Don't set it published unless there's
		# a question
		$pub = ($S->cgi->param('question')) ? 0 : 1;

		} 
	my $set = qq| qid = $newqid, unpublished = $pub, question = $question, post_date=$current_time|; 
	...
	$sth->finish;
	$S->cache->remove("poll_hash_$unqid");
	$S->cache->remove("poll_answers_$unqid");
...
sub _update_votes {
	...
	my $waid = ($S->cgi->param('aid')) ? 'aid' : "aid_$qid";
	if ($S->_is_poll_multiple_choice($qid)) {
		$aidclause = 'aid in ('.join (', ', map { $S->{DBH}->quote($_) } $S->{CGI}->param($waid)).')';
	} else {
		$aidclause = 'aid = '.$S->{DBH}->quote($S->{CGI}->param($waid));
	}
	...
	$sth->finish;
	$S->cache->remove("poll_hash_$qid");
	$S->cache->remove("poll_answers_$qid");
	$userid = $S->{REMOTE_IP} if ($userid < 0);
	$S->cache->remove("can_vote_${userid}_$qid");
