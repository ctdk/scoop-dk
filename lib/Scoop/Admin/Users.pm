...
sub get_uid_from_nick {
	...
	my $snick = $nick;
	$snick =~ s/ /_/g;
	my $id = $S->cache->fetch("ufn2_$snick");
	return $id if($id);
	my $qnick = $S->{DBH}->quote($nick);
	
	my ($rv, $sth) = $S->db_select({
		WHAT => 'uid',
		FROM => 'users',
		WHERE => qq|nickname = $qnick|});
	
	return undef unless ($rv);
	$id = $sth->fetchrow();
	$sth->finish;
	$S->cache->store("ufn2_$snick", $id);
	return $id;
}
...
sub get_nick_from_uid {
	...
	my $nick = $S->cache->fetch("nfu_$uid");
	return $nick if($nick);
...
sub calc_autoban {
	...
	my $uid = shift;
	my $mojo = shift;
	return if !$S->{UI}->{VARS}->{use_autoban};

	## REDACTED. We all have secrets, this is one of mine.
	return;
