...
sub _get_user_files {
	...
	if ( $S->{CGI}->param('confirm_delete') && $S->{CGI}->param('delete') && $file_name ) {
		...
		$S->run_hook('file_delete', $path, $file_name);
	...
	if ( $S->{CGI}->param('rename_filename') && $S->{CGI}->param('rename') && $file_name ) {
		...
		if (rename "$path$file_name", "$path$file_name_new") {
			...
			$S->run_hook('file_rename', $path, $file_name, $file_name_new);
		} else {
...
sub pref {
	my $S     = shift;
	my $key   = shift;
	# hacking in the old behavior of saving prefs :-(
	my $value = shift;

	return unless defined $key;
	# recreating the old behavior of using $S->pref() to save a pref
	# if you provide a value in addition to a key. I have no idea why
	# this changed.
	if($value){
		my $user = $S->user_data($S->{UID});
		my $return = $S->_save_pref($user, $key, $value);
		return $return;
		}
...
sub clear_prefs {
	...
	if( ref($prefs) eq 'ARRAY' ) {
		...
		for my $p ( @$prefs ) {
			...
			#my $zz = $S->cache->fetch("ud_" . $S->{UID});
                        #$zz->{prefs}->{$p} = undef;
                        #$S->cache->store("ud_" . $S->{UID}, $zz);
		}
		$where =~ s/OR $/\)/;
