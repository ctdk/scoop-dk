...
sub sub_update_type {
	...
	my $set;
	foreach (qw(perm_group_id cost max_time renewable description duration)) {
		$set .= ',' if $set;
...
sub sub_insert_type {
	...
	my $values;
	foreach (qw(type perm_group_id cost max_time renewable description duration)) {
		$values .= ',' if $values;
	...
	my ($rv, $sth) = $S->db_insert({
		...
		COLS   => 'type, perm_group_id, cost, max_time, renewable, description, duration',
		VALUES => $values
	});
	$sth->finish();
	return '' if $rv;
...
sub sub_admin_page {
	...
	my $dur_list = $S->sub_duration_list($type_data);
...
<table border=0 cellpadding=5 cellspacing=0 align="center">
	...
	<tr>
		<td valign="top">%%norm_font%%<b>Duration:</b>%%norm_font_end%%</td>
		<td valign="top">%%norm_font%%$dur_list<br><i>How long, in months, years, or forever, should the subscription be measured?</i>%%norm_font_end%%</td>
	</tr>
        <tr>
                <td valign="top">%%norm_font%%<b>Price:</b>%%norm_font_end%%</td>
		<td valign="top">%%norm_font%%<input type="text" name="cost" value="$type_data->{cost}" size=10><br><i>Per unit</i>%%norm_font_end%%</td>
        </tr>

	<tr>
		<td valign="top">%%norm_font%%<b>Maximum Time:</b>%%norm_font_end%%</td>
		<td valign="top">%%norm_font%%<input type="text" name="max_time" value="$type_data->{max_time}" size=10><br><i>In integer units, leave blank for unlimited</i>%%norm_font_end%%</td>
	</tr>
	<tr>
...
sub sub_duration_list {
        ...
        my @durations = ("month", "year", "forever");
	...
	#        }
        ...
        foreach my $d (sort @durations) {
                my $s = ($type_data->{duration} eq $d) ? ' SELECTED' : '';
                $list.= qq|
	          <option value="$d"$s>$d|;
          }
