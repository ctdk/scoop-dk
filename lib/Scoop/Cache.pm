...
sub fetch {
	...
	if($self->{scoop}->{MEMCACHED} && $resource ne 'boxes'){
		...
		# simpler.
		my $res = $S->{CONFIG}->{site_id} . "-" . $resource;
	        $res =~ s/ /^^/g;
		my $val = $S->memcached->get($res);
		return $val;
		}
...
sub store {
	...
	if($self->{scoop}->{MEMCACHED} && $resource ne 'boxes'){
		...
		my $res = $S->{CONFIG}->{site_id} . "-" . $resource;
		$res =~ s/ /^^/g;
		$S->memcached->set($res, $value, $expires);
		return 1;
		}
...
sub remove {
	...
	if($self->{scoop}->{MEMCACHED} && $resource ne 'boxes'){
		...
		my $res = $self->{scoop}->{CONFIG}->{site_id} . "-" . $resource;
		$res =~ s/ /^^/g;
		$self->{scoop}->{MEMCACHED}->delete($res);
		return 1;
		}
	return unless $self->{cache}->{data}->{$resource};
...
sub stamp {
	...
	# this isn't really applicable for this, since there's no real 
	# analog to it in memcached
	#if($self->{scoop}->{MEMCACHED}){
	#	warn "We would be stamping $resource here\n" if $DEBUG;
		# fuck it, just return the time and make Scoop think it's
