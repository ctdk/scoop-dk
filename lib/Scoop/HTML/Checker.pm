...
sub clean_html {
	...
	elsif ($context eq 'sig') { # special case for signatures
 		$self->{shtml} = $self->generate_allowed_html('sig_html')
			unless $self->{shtml};
		$self->{ahtml_bak} = $self->{ahtml};
                $self->{ahtml} = $self->{shtml};
		$self->{use_sig} = 1;
...
sub _tag_start {
	...
	if ($self->{ahtml}->{$tag} && $self->_check_context($tag)) {
		...
		# Make a var to check if we're using XML style closing (a la
                # <br /> and the like) with this tag
                my $xmlclose = undef;
		foreach my $a (keys %{$args}) {
			...
			if (($tag eq 'a') && ($a eq 'href')) {
				if ($args->{$a} ne $self->{scoop}->check_url($args->{$a})) {
					push(@{ $self->{errors} }, "Value ($args->{$a}) for attribute " . $a . " in tag " . $tag . " is $err_words");
				}
			...
			if ($self->{ahtml}->{$tag}->{$a}) {
				...
				# OK, this is where the special code shit
				# has to go.
					if ($tag eq 'img' && $a eq 'src' && !$self->{scoop}->have_perm('evade_imagewl')){
						my $wlist = $self->{scoop}->{UI}->{BLOCKS}->{image_whitelist};
						my @imgwhitelist = split /\n/, $wlist;
						my $whitelist = join '|', @imgwhitelist;
						# oh, it had better fucking not be
						$whitelist =~ s/\r//g;
						if($args->{$a} =~ m#$whitelist#){
							$new_tag .= $a;
							$new_tag .= qq~="$args->{$a}"~;
							$new_tag .= " ";
							}
						else {
							my $approvedomains = join ', ', @imgwhitelist;
							$approvedomains =~ s/\r//g;
							push(@{ $self->{errors} }, "$args->{$a} is not from an approved image host. The approved domains are: $approvedomains.");
							}
						}
					else {
						$new_tag .= $a;
			...
			# constraints on its value
				...
				$new_tag .= qq~="$args->{$a}"~ if defined($args->{$a});
				$new_tag .= " ";
			# special style & perm case. w00!
                        } elsif ($a eq 'style' && $self->{scoop}->have_perm('use_style_attr')){
                                $new_tag .= $a;
                                $new_tag .= qq~="$args->{$a}"~ if defined($args->{$a});
                                $new_tag .= " ";
			# if it's here, then the argument isn't allowed
				...
				unless ($self->{nacache}->{$tag}->{$a}) {
					...
					push(@{ $self->{errors} }, "Attribute " . $a . " for tag " . $tag . " is $err_words");
		...
		$new_tag .= "$xmlclose>";  	# and close the tag

		# before we return the new tag, let's see about validating closing tags
		$self->_check_stack($tag, 0, $xmlclose);   # 0 means not a closing tag

		$parser->{result} .= $new_tag;
	} else {
		if ($self->{use_all}) {
			unless ($self->{nacache}->{$tag}) {
				push(@{ $self->{errors} }, "Tag " . $tag . " is unknown");
				$self->{nacache}->{$tag} = 1;
			}
		} elsif ($self->{use_sig}) {
			unless ($self->{nacache}->{$tag}) {
			        push(@{ $self->{errors} }, "Tag " . $tag . " is not allowed.");
				$self->{nacache}->{$tag} = 1;
		       }
		} else {
		...
		if ($closing) {
			...
			if (!$last_tag) {
				...
				$i = lc($i);
                		...
                		elsif($i eq 'b' || $i eq 'strong'){
                        	$i = "&lt;/strong&gt; or &lt;/b&gt;";
                        		}
                		else {
                        		$i = "&lt;/$i&gt;";
                        		}
				push(@{ $self->{errors} }, "No opening tag found for closing tag " . $i);
			} elsif ($tag ne $last_tag) {
				push(@{ $self->{errors} }, "Closing tag (" . $tag . ") does not match last opening tag (" . $last_tag . ").");
			}
		} else {
			push(@{ $self->{stack} }, $tag) unless $xmlclose;  # save it to check later
		}
	}
}
...
sub empty_stack {
	...
	foreach my $i (@{ $self->{stack} }) {
		...
		push(@{ $self->{errors} }, "No closing tag found for opening tag " . $i);
	}
}
...
sub errors_as_string {
	...
	my $string = "Your HTML has the following $errors$where: <ul>\n";
