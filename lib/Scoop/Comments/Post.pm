...
sub post_form {
	...
	if ($preview) {
		...
		#	sig => $cur_sig };
                ...
                $pre_comment->[2] = $pid;
                $pre_comment->[3] = 'soon';
                $pre_comment->[21] = 'Preview';
                $pre_comment->[6] = $f_subject;
                $pre_comment->[7] = $f_comment;
                $pre_comment->[8] = $uid;
                $pre_comment->[12] = $pending;
                $pre_comment->[13] = 1;
                $pre_comment->[14] = $cur_sig;
...
sub filter_comment {
	...
	if ($postmode eq 'auto') {
        ...
        $comment = $S->url_shorten($comment);

	# if it's not a
	$comment =~ s/&(?![A-Za-z0-9#]+;)/&amp;/g;

        # weeeirdo thing with params. It needs to be able to be submitted as
        # <param ...></param> or <param ... />. Convert the second to the
        # first for now. The HTML checker needs to be redone, because it makes
        # little sense.
        #$comment =~ s#<param(.*?)/>#<param$1></param>#gi;
	...
	# *NOW* we have to put the XML style /> closing stuff on certain
	# tags *BACK*, because for some reason trying to keep it in the
	# HTML parser kills everything
	#$comment =~ s/<img(.*?)>/<img$1 \/>/g;
	#$comment =~ s/<br>/<br \/>/g;
	#$comment =~ s/<hr>/<hr \/>/g;

	# Voterpunch "plugin" stuff (not too much of a plugin, though, seeing
	# as it requires a code patch). Oh well, it's fairly simple.
#	if($S->{UI}->{VARS}->{use_voterpunch} && $S->have_perm('use_vp_code')){
#            my $js = $S->{UI}->{BLOCKS}->{voterpunch_js};
#	    while($comment =~ /PP{{(.*?)}}/g{
#                my $critter = $1;
#		my $clink = $S->urlify($critter);
#		$js =~ s/__CRITTERLINK__/$clink/g;
#		$js =~ s/__CRITTER__/$critter/g;
#		$comment =~ s#PP{{(.*?)}}#$sub#;
#	        }
#	    }
...
sub post_comment {
	...
	my $points = undef;
	my $set_rating = 0;
	my $cols = 'sid, pid, date, subject, comment, uid, pending, sig_status, sig, commentip, story_id';
	my $vals = qq|$f_sid, $pid, NOW(), $subject, $comment, $uid, $pending, $sig_status, $sig, $commentip, $qs_id|;
	...
	$S->run_hook('comment_new', $sid, $cid, $subject, $comment);
