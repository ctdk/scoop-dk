...
sub db_select {
	...
	# Determine if we're gonna use the search db or not. If we are using
	# it, we don't need to worry about looking in the archive.
	if ($args->{SEARCH}){
		$search = 1;
		if(!$S->{DBHSEARCH}){
			my $dbname = $S->{UI}->{VARS}->{db_name_search};
			my $username = $S->{UI}->{VARS}->{db_user_search};
			my $password = $S->{UI}->{VARS}->{db_pass_search};
			my $dbhost = $S->{UI}->{VARS}->{db_host_search};
			my $dbtype = 'mysql';
			my $data_source = "DBI:$dbtype:dbname=$dbname;host=$dbhost";
			$S->{DBHSEARCH} =  DBI->connect($data_source, $username, $password) ||
...
sub db_insert {
	...
	$query .= " VALUES ($args->{VALUES})" unless $args->{SELECT};

	if ($args->{SELECT}) {
		$query .= " $args->{SELECT}";
		}
	if($args->{DUPLICATE}) {
                $query .= " ON DUPLICATE KEY UPDATE $args->{DUPLICATE}";
                }
...
sub db_start_transaction {
	...
	unless ($S->{CONFIG}->{mysql_version} =~ /^4/ || $S->{CONFIG}->{mysql_version} =~ /^5/) {
	...
	my $rc = $dbh->begin_work;
        warn "<<ERROR>> in db_start_transaction: $DBI::errstr\n" unless $rc;
        return $rc;
}
...
sub db_commit {
	...
	unless ($S->{CONFIG}->{mysql_version} =~ /^4/ || $S->{CONFIG}->{mysql_ve
rsion} =~ /^5/) {
                return "Transactions not supported in this version of MySQL.\n";
                }
        $Scoop::DB_QUERY_COUNT++ if ($Scoop::COUNT_DEBUG);
	...
	my $rc = $dbh->commit;
        ...
        warn "<<ERROR>> in db_commit: $DBI::errstr\n" unless $rc;
        return $rc;
}
...
rsion} =~ /^5/) {
        ...
        return('0E0',undef) unless ($dbh);
	my $rc = $dbh->rollback;
        warn "<<ERROR>> in db_rollback: $DBI::errstr\n" unless $rc;
        return $rc;
}
...
sub create_temp_table {
	...
	# temp tables are specific to a connection, so this is as good as
	# anything else
	my $tempname = $base . "_" . time(); 
	my $dbh;
	if($archive){
		$dbh = $S->get_archive_dbh();
		}
	...
	my $query = "CREATE TEMPORARY TABLE $tempname LIKE $base";
	my $sth = $dbh->prepare($query);
        my $rv = $sth->execute();
	# check this better later:
	warn "Eep, temp table wasn't created: $DBI::errstr\n" unless $rv;
