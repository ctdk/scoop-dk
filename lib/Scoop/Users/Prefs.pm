	...
	if ( $page eq 'Protected' ) {
		...
		if ( $S->have_perm('edit_user') || $S->check_password($nick,$pass) ) {
			...
			if ( $save{nickname} ) {
				...
				$sth2->finish;
				$S->cache->remove("ufn_$nick");
				$S->cache->remove("nfu_$nick");
			}
...
sub _save_pref {
	...
	if ( $pref_item->{html} ) {
		my $prefflag = ($pref eq 'sig') ? 'sig' : 'prefs';
		$value = $S->filter_comment($value, $prefflag);
	...
	if ( defined $user->{prefs}->{$pref} ) {
		...
		# You know, we could change a lot of update || insert
		# statements to use ON DUPLICATE KEY UPDATE
		($rv, $sth) = $S->db_insert({
			DEBUG => $DEBUG,
			INTO  => 'userprefs',
			COLS  => 'uid, prefname, prefvalue',
			VALUES => "$uid, $q_key, $q_value",
			DUPLICATE => "prefvalue = $q_value"
		});
		$sth->finish;
	}
	if ($rv) {
...
sub _reset_user_prefs {
	...
	foreach my $pref (@preflist) {
		...
		#$S->cache->remove("ud_$uid");
		#my $zz = $S->cache->fetch("ud_$uid");
                #delete $zz->{prefs}->{$pref};
                #$S->cache->store("ud_$uid", $zz);
