...
#	$comments = $S->display_comments($qid, '0');
	...
	if( $S->get_sid_from_qid($qid) ) {
		...
		# and make a link back
		my $s = $S->story_data_arr([$qid]);
		my $story = $s->[0];
		my $pollstory = qq|Poll submitted: <i>$story->[31]</i><br />Back to <a href="/storyonly/$story->[0]">$story->[3]</a>.|;
		$S->{UI}->{BLOCKS}->{STORY} .= $pollstory;
	} else {
...
sub display_poll {
	...
	if( $tovote && $tovote eq "Vote"  && $S->_can_vote($qid)) {
		...
		unless ( $S->{CGI}->param("aid_$qid") eq "" ) {
			# otherwise call vote handler	
			$S->_update_votes($qid, $S->{CGI}->param("aid_$qid"));
		}
	}
