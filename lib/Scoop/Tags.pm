...
sub save_tags {
	...
	# get version
	my $ver;
	if($S->{UI}->{VARS}->{use_tag_versioning}){
		my ($rv, $sth) = $S->db_select({
        		WHAT => 'version',
                	FROM => 'story_tags_ver',
                	WHERE => qq|sid = $q_sid|,
	...
	foreach my $tag (@tags) {
		...
		$sth->finish();
		# tag versioning
		# it would probably be nice to have this stuff in the
		# actual story_tags table, but it's big enough as it is.
		# Hopefully having it separate will reduce strain on the
		# machine.
		if($S->{UI}->{VARS}->{use_tag_versioning}){
			my $uid = $S->{UID};
			($rv, $sth) = $S->db_insert({
				INTO => 'story_tags_ver',
				COLS => 'sid, tag, tag_order, uid, version',
				VALUES => qq{$q_sid, $q_tag, $i, $uid, $ver}
				});
			$sth->finish;
			# clean out old entries, if we want to.
			if($S->{UI}->{VARS}->{max_archived_versions}){
				my $del = $ver - $S->{UI}->{VARS}->{max_archived_versions};
				if($del > 0){
					($rv, $sth) = $S->db_delete({
						FROM => 'story_tags_ver',
						WHERE => "sid = $q_sid AND version <= $del"
						});
					$sth->finish;
					}
				}
...
sub get_all_tags {
	...
	if(my $retcache = $S->cache->fetch('tags_'.$sort."_$args->{limit}"."_$args->{cutoff}")){
		warn "get_all_tags getting from cache" if $DEBUG;
		return $retcache;
		}
	...
	# cache that bad boy
	$S->cache->stamp('tags_'.$sort."_$args->{limit}"."_$args->{cutoff}");
	$S->cache->store('tags_'.$sort."_$args->{limit}"."_$args->{cutoff}", \@return, "+15m");
	warn "get_all_tags caching....\n" if $DEBUG;
...
sub show_storiesbytag {
	...
	elsif($S->cgi->param('caller_op') eq 'alltags') {
		$S->{UI}{BLOCKS}{subtitle} = qq{All Tags};
		return $S->all_tags_page({displaystatus => [0,1]});
	}else{
		$S->{UI}{BLOCKS}{subtitle} = qq{Top 150 Tags};
		return $S->all_tags_page({cutoff => 10, displaystatus => [0,1], limit => 150});
	}
}
