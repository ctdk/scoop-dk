...
sub handler {
	...
	&_parse_path($S);
        ...
        my $rqdebugtime = time() if $REQDEBUG;
        my $rd;
	my $ruid;
        if($REQDEBUG){
                $rd = $S->apache->uri();
		$ruid = $S->{UID};
                warn "REQDEBUG: Starting handing $rd from $$ for uid $ruid at $rqdebugtime\n";
                }

	if($S->{UID} == -1 && $S->{UI}->{VARS}->{expercache} && ! $S->{SESSION_KEY} && $S->apache->method() eq 'GET'){ # hmm?
		if ($S->cgi->param('op') eq 'displaystory' || $S->cgi->param('op') eq 'main' || $S->cgi->param('op') eq 'comments'){
			my $uri = $S->apache->uri();
			if (my $mcpage = $S->cache->fetch("mcuri_$uri")){
				$S->{APACHE}->headers_out->{'Content-Length'} = length($mcpage);
				$S->apache->content_type('text/html; charset='.($S->{UI}->{VARS}->{charset} || 'ISO-8859-1'));
				my $start = 0;
				my $len = 63000;
				while (my $p = substr($mcpage, $start, $len)) {
					$start += $len;
					$S->apache->print($p);
					}
				$S->apache->rflush();
	...
	if ($S->{UI}->{VARS}->{safe_mode} && ($S->{GID} ne 'Superuser') && !($S->have_perm('bypass_safe_mode'))) {
		...
		if($redir_page) {
			$S->apache->headers_out->add('Location' => $redir_page);
	...
	# figure out caching stuff. For performance, we'd like the option
	# of sending expires headers to anonymous clients for certain
	# pages
	# Oh, and CSS stuff should have expiration dates too, probably,
	# just to be on the safe side
	if(($S->{UID} < 0 && $S->{UI}->{VARS}->{use_cache_anon}) || $op eq 'css'){
		use POSIX qw (strftime);
		my $exp = ($op eq 'css') ? $S->{UI}->{VARS}->{css_expiration} : $S->{UI}->{VARS}->{anon_expiration};
		my $expstr = strftime "%a, %b %e %H:%M:%S %Y GMT", gmtime(time + $exp);
		$S->apache->headers_out->add('Expires' => $expstr);
		$S->{ALLOW_CACHING} = 1;
		# should be it
		}
	...
	if($REQDEBUG){
		my $f = time();
		my $took = $f - $rqdebugtime;
		warn "REQDEBUG: Finishing $rd from $$ for UID $ruid at $f, started at $rqdebugtime, took $took\n";
		}
