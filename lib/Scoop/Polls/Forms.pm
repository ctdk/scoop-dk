...
sub make_attached_poll_form {
	...
	if( $action eq 'preview' ) {
		my $params = shift;
		# yeah, this isn't weird at all, is it?
		if($params->{sid}){
			$qid = $S->get_qid_from_sid($params->{sid});
			return '' if ($S->_does_poll_exist($qid) && !$S->have_perm('story_admin'));
			}
		$qid = $params->{qid};
	} else {
		my $sid = shift;
	...
	# bug out if we're a mere mortal and the poll already exists
	if($S->_does_poll_exist($qid) && !$S->have_perm('story_admin')){
		# assuming the poll's unpublished
		my $ph = $S->get_poll_hash($qid);
		return '' if !$ph->{unpublished};
		}
