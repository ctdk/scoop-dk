...
sub get_poll_hash {
	...
	if( $action eq 'preview' ) {
		...
		#unless ($return_hash = $S->cache->fetch("poll_hash_$qid")){
		$return_hash = $S->cache->fetch("poll_hash_$qid");
		unless ($return_hash->{qid}){
			($rv, $sth) = $S->db_select({
				DEBUG   => 0,
				WHAT	=> "qid,question,voters,post_date,unpublished",
				FROM	=> 'pollquestions',
	...
	# store it!
	$S->cache->store("poll_hash_$qid", $return_hash);
	return $return_hash;
...
sub get_poll_answers {
	...
	if( $action eq 'preview' ) {
		...
		unless ($answers = $S->cache->fetch("poll_answers_$unqid")){
			($rv, $sth) = $S->db_select({
				DEBUG 	 => 0,
			...
			unless ( $#{$answers} > 0 ) {
				...
				$sth->finish();
				$S->cache->store("poll_answers_$unqid", $answers);
				}
			}
	}
...
sub get_sid_from_qid {
	...
	my $sid;
	return 0 if !$qid;
	if($sid = $S->cache->fetch("sfq_$qid")){
		return $sid;
		}

	my $qqid = $S->{DBH}->quote($qid);
	my ($rv, $sth) = $S->db_select({
		DEBUG	=> 0,
		FROM	=> 'stories',
		WHAT	=> 'sid, displaystatus',
		WHERE	=> qq| attached_poll = $qqid |
	});
	...
	if ($rv) {
		...
		$sid = $tmphash->{sid};
		if($tmphash->{displaystatus} >= 0){
			$S->cache->store("sfq_$qid", $sid);
			}
		return $sid;
...
sub _can_vote {
	my $S = shift;
	my $qid = shift;
	my $uid = ($S->{UID} > 0) ? $S->{UID} : $S->{REMOTE_IP};

	return 0 unless ($S->have_perm('poll_vote')); 

	if(my $v = $S->cache->fetch("can_vote_${uid}_$qid")){
		return ($v eq '0E0') ? 0 : 1;
		}
	#return 0 if ($S->_is_poll_archived($qid));
	if ($S->_is_poll_archived($qid)){
		$S->cache->store("can_vote_${uid}_$qid", '0E0');
		return 0;
		}
	...
	# warn "Returning 1\n";
	$S->cache->store("can_vote_${uid}_$qid", 1);
	return 1;
}
